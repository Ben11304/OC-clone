...
function huggingFaceDatasetRepo(ds){
  ...
  try {
    ...
    if (!['huggingface.co', 'www.huggingface.co'].includes(url.hostname.toLowerCase())) return null;
    const parts = url.pathname.split('/').filter(Boolean).map(part => decodeURIComponent(part));
    if (parts[0] !== 'datasets' || parts.length < 3) return null;
    const repoId = `${parts[1]}/${parts[2]}`;
    if (!/^[A-Za-z0-9._-]+\/[A-Za-z0-9._-]+$/.test(repoId)) return null;
    return repoId;
  } catch {
    return null;
  }
...
function datasetDownloadInstructions(ds){
  const methods = Array.isArray(ds?.programmatic_access)
    ? ds.programmatic_access.filter(item => item && typeof item === 'object')
    : [];
  const repoPattern = /^[A-Za-z0-9._-]+\/[A-Za-z0-9._-]+$/;
  const datasetDir = String(ds?.id || ds?.name || 'dataset')
    .trim()
    .replace(/[^A-Za-z0-9._-]+/g, '-')
    .replace(/^-+|-+$/g, '') || 'dataset';

  const baiduMethod = methods.find(item => item.method === 'baidu_share_transfer');
  if (baiduMethod) {
    const shareUrl = safeHref(baiduMethod.share_url || ds?.access || '');
    const extractionCode = String(baiduMethod.extraction_code || '').trim();
    let validShare = false;
    try {
      const parsed = new URL(shareUrl);
      validShare = ['pan.baidu.com', 'www.pan.baidu.com'].includes(parsed.hostname.toLowerCase())
        && /^\/s\/[A-Za-z0-9_-]+\/?$/.test(parsed.pathname);
    } catch {}
    if (validShare && /^[A-Za-z0-9]{4,8}$/.test(extractionCode)) {
      const browserUrl = `${shareUrl}${shareUrl.includes('?') ? '&' : '?'}pwd=${encodeURIComponent(extractionCode)}`;
      return {
        provider: 'Baidu Pan',
        repoId: new URL(shareUrl).pathname.split('/').filter(Boolean).pop() || datasetDir,
        repoUrl: browserUrl,
        description: 'Baidu Pan does not expose a permanent public file URL. This shortcut uses the community BaiduPCS-Go client to transfer the shared dataset into the user’s authenticated account and immediately download it locally.',
        tabs: [{
          id: 'cli',
          label: 'CLI',
          help: 'Install BaiduPCS-Go from its Releases page, sign in to your own Baidu account, then run the transfer command.',
          command: [
            'BaiduPCS-Go login',
            `BaiduPCS-Go config set -savedir ${JSON.stringify(`./${datasetDir}`)}`,
            `BaiduPCS-Go transfer ${JSON.stringify(shareUrl)} ${JSON.stringify(extractionCode)} --download`
          ].join('\n'),
          copyLabel: 'Copy commands'
        }],
        notice: 'BaiduPCS-Go is a community client, not an official OpenConstruction or Baidu tool. Never paste BDUSS, STOKEN, cookies, or passwords into OpenConstruction; authenticate only inside your local CLI. Baidu account, quota, region, or anti-abuse checks may still block the transfer.',
        links: [
          { url: browserUrl, label: `Open Baidu Pan (code: ${extractionCode})` },
          { url: 'https://github.com/qjfoidnh/BaiduPCS-Go/releases', label: 'Download BaiduPCS-Go' },
          { url: 'https://github.com/qjfoidnh/BaiduPCS-Go#%E8%BD%AC%E5%AD%98%E6%96%87%E4%BB%B6%E7%9B%AE%E5%BD%95', label: 'Transfer documentation' }
        ]
      };
    }
  }

  const httpFilesMethod = methods.find(item => item.method === 'http_files');
  if (httpFilesMethod) {
    const files = (Array.isArray(ds?.distribution) ? ds.distribution : [])
      .map(item => ({
        url: safeHref(item?.content_url || ''),
        filename: String(item?.filename || '').trim()
      }))
      .filter(item => item.url && /^[A-Za-z0-9._-]+$/.test(item.filename));
    if (files.length >= 1 && files.length === ds.distribution.length) {
      const cliCommand = [
        `mkdir -p ${JSON.stringify(`./${datasetDir}`)}`,
        ...files.map(item => `curl --fail --location --continue-at - --output ${JSON.stringify(`./${datasetDir}/${item.filename}`)} ${JSON.stringify(item.url)}`)
      ].join('\n');
      const docsUrl = safeHref(httpFilesMethod.documentation_url || '');
      return {
        provider: String(httpFilesMethod.provider || files[0]?.provider || 'HTTP files'),
        repoId: datasetDir,
        repoUrl: safeHref(ds?.access || ''),
        description: 'This dataset is published as multiple independent archives. The commands download every official file into one directory and resume interrupted transfers.',
        tabs: [{
          id: 'cli',
          label: 'CLI',
          help: 'Requires curl. Re-run the same commands to resume any partially downloaded archive.',
          command: cliCommand,
          copyLabel: 'Copy commands'
        }],
        notice: String(httpFilesMethod.notice || '').trim()
          || 'Keep all archives together. Some providers separate train, validation, and test data or publish different modalities as individual files.',
        links: [
          ...(safeHref(ds?.access || '') ? [{ url: safeHref(ds.access), label: 'Open dataset page' }] : []),
          ...(docsUrl ? [{ url: docsUrl, label: 'Download documentation' }] : [])
        ]
      };
    }
  }

  const dreamHouseMethod = methods.find(item => item.method === 'dreamhouse_setup');
  if (dreamHouseMethod && ds?.id === 'DreamHouse') {
    const repoUrl = 'https://github.com/SalesforceAIResearch/DreamHouse';
    return {
      provider: 'DreamHouse CLI',
      repoId: 'SalesforceAIResearch/DreamHouse',
      repoUrl,
      description: 'The official DreamHouse package downloads the split benchmark artifacts, reassembles the task pack, verifies its checksum, and installs the validator artifact.',
      tabs: [{
        id: 'cli',
        label: 'CLI',
        help: 'Requires Python 3.9 or newer. Run these commands in the environment where you will use the benchmark.',
        command: 'python -m pip install -U dreamhouse\ndreamhouse setup --download-artifacts',
        copyLabel: 'Copy commands'
      }],
      notice: 'Blender is only required to execute the benchmark. Add --install-blender to the setup command if you also want the tested Blender runtime.',
      links: [
        { url: repoUrl, label: 'Open repository' },
        { url: 'https://github.com/SalesforceAIResearch/DreamHouse#quick-start', label: 'Setup documentation' }
      ]
    };
  }

  const googleDriveMethod = methods.find(item => item.method === 'google_drive_folder');
  if (googleDriveMethod) {
    const folderId = String(googleDriveMethod.folder_id || '').trim();
    if (/^[A-Za-z0-9_-]+$/.test(folderId)) {
      const folderUrl = `https://drive.google.com/drive/folders/${folderId}`;
      const cliCommand = `python -m pip install -U google-api-python-client
export GOOGLE_DRIVE_API_KEY="replace-with-your-api-key"
python - <<'PY'
import io
import os
import pathlib
...
destination = pathlib.Path(${JSON.stringify(datasetDir)})
api_key = os.environ.get("GOOGLE_DRIVE_API_KEY", "")
if not api_key or api_key == "replace-with-your-api-key":
    raise SystemExit("Set GOOGLE_DRIVE_API_KEY to a Google Drive API key first.")

drive = build("drive", "v3", developerKey=api_key, cache_discovery=False)
folder_mime = "application/vnd.google-apps.folder"

def safe_name(value):
    name = pathlib.Path(str(value)).name
...
def download_folder(parent_id, local_dir):
    ...
    for item in list_children(parent_id):
        ...
        if item["mimeType"].startswith("application/vnd.google-apps."):
            print(f"Skipping Google-native file that needs export: {target}")
            continue
...
print(f"Downloaded public folder to {destination.resolve()}")
PY`;
      return {
        provider: 'Google Drive',
        repoId: folderId,
        repoUrl: safeHref(ds?.access || '') || folderUrl,
        description: 'Google Drive does not expose one stable archive URL for a shared folder. This command uses the official Drive API to enumerate every page and download the complete public folder recursively.',
        tabs: [{
          id: 'cli',
          label: 'CLI',
          help: 'Create a Google Cloud API key with the Drive API enabled, replace the placeholder, then run the commands.',
          command: cliCommand,
          copyLabel: 'Copy commands'
        }],
        notice: 'A browser-generated folder ZIP is temporary, and gdown can omit files when a folder contains more than 50 children. The paginated Drive API route is used here so an MCP agent can retrieve the complete dataset.',
        links: [
          { url: safeHref(ds?.access || '') || folderUrl, label: 'Open shared folder' },
          { url: 'https://developers.google.com/workspace/drive/api/guides/manage-downloads', label: 'Google Drive API documentation' }
        ]
      };
    }
  }

  const figshareMethod = methods.find(item => item.method === 'figshare_files');
  if (figshareMethod) {
    const recordId = String(figshareMethod.record_id || '').trim();
    const apiUrl = safeHref(figshareMethod.api_url || `https://api.figshare.com/v2/articles/${recordId}`);
    let validApi = false;
    try {
      validApi = new URL(apiUrl).hostname.toLowerCase() === 'api.figshare.com';
    } catch {}
    if (/^\d+$/.test(recordId) && validApi) {
      const cliCommand = `python - <<'PY'
import json
import pathlib
import shutil
...
request = urllib.request.Request(api_url, headers={"User-Agent": "OpenConstruction/1.0"})
with urllib.request.urlopen(request) as response:
    article = json.load(response)

files = article.get("files", [])
if not files:
    raise SystemExit("No public files were returned by the Figshare API.")

for item in files:
    name = pathlib.Path(str(item["name"])).name
    target = destination / name
    print(f"Downloading {name}")
    request = urllib.request.Request(item["download_url"], headers={"User-Agent": "OpenConstruction/1.0"})
    with urllib.request.urlopen(request) as source, target.open("wb") as output:
        shutil.copyfileobj(source, output)

print(f"Downloaded {len(files)} files to {destination.resolve()}")
PY`;
      return {
        provider: 'Figshare',
        repoId: recordId,
        repoUrl: safeHref(ds?.access || '') || `https://figshare.com/articles/${recordId}`,
        description: 'Figshare exposes this dataset as multiple public files. This command reads the official article API and downloads every file into one local directory.',
        tabs: [{
          id: 'cli',
          label: 'CLI',
          help: 'Requires Python 3 and no third-party packages.',
          command: cliCommand,
          copyLabel: 'Copy command'
        }],
        notice: 'Keep the terminal open until every file finishes. The command preserves the filenames returned by the selected article version.',
        links: [
          { url: safeHref(ds?.access || '') || `https://figshare.com/articles/${recordId}`, label: 'Open dataset page' },
          { url: 'https://docs.figshare.com/old_docs/api/articles/', label: 'Figshare API documentation' }
        ]
      };
    }
  }

  const dataverseMethod = methods.find(item => item.method === 'dataverse_collection');
  if (dataverseMethod) {
    const collectionId = String(dataverseMethod.collection_id || '').trim();
    const apiUrl = safeHref(dataverseMethod.api_url || 'https://dataverse.harvard.edu/api').replace(/\/$/, '');
    let validApi = false;
    try {
      validApi = new URL(apiUrl).hostname.toLowerCase() === 'dataverse.harvard.edu';
    } catch {}
    if (/^[A-Za-z0-9._-]+$/.test(collectionId) && validApi) {
      const cliCommand = `python - <<'PY'
import json
import pathlib
import shutil
...
api_root = ${JSON.stringify(apiUrl)}
root_alias = ${JSON.stringify(collectionId)}
destination = pathlib.Path(${JSON.stringify(datasetDir)})
destination.mkdir(parents=True, exist_ok=True)
seen = set()

def get_json(url):
    request = urllib.request.Request(url, headers={"User-Agent": "OpenConstruction/1.0"})
    with urllib.request.urlopen(request) as response:
        return json.load(response)["data"]

def download_dataset(item):
    persistent_id = f'{item["protocol"]}:{item["authority"]}/{item["identifier"]}'
    query = urllib.parse.urlencode({"persistentId": persistent_id})
    url = f"{api_root}/access/dataset/:persistentId/?{query}"
    filename = f'{item["identifier"]}.zip'.replace("/", "-")
    target = destination / filename
    print(f"Downloading {persistent_id}")
    request = urllib.request.Request(url, headers={"User-Agent": "OpenConstruction/1.0"})
    with urllib.request.urlopen(request) as source, target.open("wb") as output:
        shutil.copyfileobj(source, output)
...
def walk(alias):
    ...
    for item in get_json(f"{api_root}/dataverses/{encoded}/contents"):
        if item.get("type") == "dataset":
            download_dataset(item)
        elif item.get("type") == "dataverse" and item.get("identifier"):
            walk(item["identifier"])

walk(root_alias)
print(f"Downloaded the collection to {destination.resolve()}")
PY`;
      return {
        provider: 'Dataverse',
        repoId: collectionId,
        repoUrl: safeHref(ds?.access || '') || `https://dataverse.harvard.edu/dataverse/${collectionId}`,
        description: 'This catalog entry is a Dataverse collection rather than one dataset. The command enumerates its datasets recursively and downloads each published dataset bundle.',
        tabs: [{
          id: 'cli',
          label: 'CLI',
          help: 'Requires Python 3 and no third-party packages.',
        ...
        notice: 'Restricted files, guestbooks, or datasets that require acceptance will still need a Dataverse API token and explicit user action.',
        links: [
          { url: safeHref(ds?.access || '') || `https://dataverse.harvard.edu/dataverse/${collectionId}`, label: 'Open collection' },
          { url: 'https://guides.dataverse.org/en/latest/api/dataaccess.html', label: 'Dataverse API documentation' }
        ]
      };
    }
  }

  const designSafeMethod = methods.find(item => item.method === 'designsafe_globus');
  if (designSafeMethod) {
    const resourceId = String(designSafeMethod.resource_id || '').trim();
    const sourcePath = String(designSafeMethod.source_path || '').trim();
    if (/^PRJ-\d+$/.test(resourceId) && /^\/corral\/projects\/NHERI\/published\/PRJ-\d+\/$/.test(sourcePath)) {
      const cliCommand = [
        'python -m pip install -U globus-cli',
        'globus login',
        'globus endpoint search "TACC Corral3"',
        '',
        'SOURCE_COLLECTION_ID="replace-with-tacc-corral3-collection-id"',
        'DESTINATION_COLLECTION_ID="replace-with-your-collection-id"',
        `globus transfer "$SOURCE_COLLECTION_ID:${sourcePath}" "$DESTINATION_COLLECTION_ID:/${datasetDir}/" --recursive`
      ].join('\n');
      return {
        provider: 'DesignSafe / Globus',
        repoId: resourceId,
        repoUrl: safeHref(ds?.access || '') || `https://www.designsafe-ci.org/data/browser/public/designsafe.storage.published/${resourceId}/`,
        description: 'DesignSafe stores this published project on TACC Corral. Use Globus for a resumable recursive transfer of the complete project directory.',
        tabs: [{
          id: 'cli',
          label: 'CLI',
          help: 'Sign in to Globus, then replace the two collection IDs before running the transfer command.',
          command: cliCommand,
          copyLabel: 'Copy commands'
        }],
        notice: 'Authentication and a destination Globus collection are required. An MCP agent must pause and request these user-controlled credentials and destination details.',
        links: [
          { url: safeHref(ds?.access || '') || `https://www.designsafe-ci.org/data/browser/public/designsafe.storage.published/${resourceId}/`, label: 'Open published project' },
          { url: 'https://designsafe-ci.org/user-guide/managingdata/datatransfer/', label: 'DesignSafe transfer guide' },
          { url: 'https://docs.globus.org/cli/reference/transfer/', label: 'Globus CLI documentation' }
        ]
      };
    }
  }

  const roboflowMethod = methods.find(item => item.method === 'roboflow_version');
  if (roboflowMethod) {
    const workspaceId = String(roboflowMethod.workspace_id || '').trim();
    const projectId = String(roboflowMethod.project_id || '').trim();
    const version = String(roboflowMethod.version || '').trim();
    const exportFormat = String(roboflowMethod.export_format || 'coco').trim().toLowerCase();
    const slugPattern = /^[A-Za-z0-9._-]+$/;
    const formatPattern = /^[A-Za-z0-9._-]+$/;
    if (slugPattern.test(workspaceId) && slugPattern.test(projectId) && /^\d+$/.test(version) && formatPattern.test(exportFormat)) {
      const resourceId = `${workspaceId}/${projectId}/${version}`;
      const repoUrl = `https://universe.roboflow.com/${workspaceId}/${projectId}/dataset/${version}`;
      return {
        provider: 'Roboflow',
        repoId: resourceId,
        repoUrl,
        description: 'Roboflow generates a versioned export containing the images, splits, and annotations. Use the official CLI with an API key so the export can be requested and downloaded by an agent.',
        tabs: [{
          id: 'cli',
          label: 'CLI',
          help: 'Install the Roboflow package, authenticate with your API key, and download the selected public Universe version.',
          command: [
            'python -m pip install -U roboflow',
            'export ROBOFLOW_API_KEY="replace-with-your-api-key"',
            `roboflow download ${resourceId} -f ${exportFormat} -l ./${datasetDir}`
          ].join('\n'),
          copyLabel: 'Copy commands'
        }],
        notice: 'Roboflow export URLs are generated dynamically and can expire. Keep the API key private; an MCP agent should read it from a secret or environment variable rather than catalog metadata.',
        links: [
          { url: repoUrl, label: 'Open dataset version' },
          { url: 'https://docs.roboflow.com/datasets/create-and-upload/download-a-dataset', label: 'Roboflow download documentation' }
        ]
      };
    }
  }

  const kaggleMethod = methods.find(item => item.method === 'kaggle_competition');
  if (kaggleMethod) {
    const competitionId = String(kaggleMethod.competition_id || '').trim();
    if (/^[A-Za-z0-9._-]+$/.test(competitionId)) {
      const repoUrl = `https://www.kaggle.com/competitions/${competitionId}/data`;
      return {
        provider: 'Kaggle',
        repoId: competitionId,
        repoUrl,
        description: 'Kaggle competition data is downloaded through the official CLI after the user joins the competition and accepts its rules.',
        tabs: [{
          id: 'cli',
          label: 'CLI',
          help: 'Sign in on Kaggle, accept the competition rules, configure a Kaggle API token, then run these commands.',
          command: [
            'python -m pip install -U kaggle',
            `mkdir -p ${JSON.stringify(`./${datasetDir}`)}`,
            `kaggle competitions download ${competitionId} -p ${JSON.stringify(`./${datasetDir}`)}`
          ].join('\n'),
          copyLabel: 'Copy commands'
        }],
        notice: 'A 401 response means the Kaggle token is missing or the account has not accepted the competition rules. Open the competition page and complete that user-controlled step before retrying.',
        links: [
          { url: repoUrl, label: 'Open competition data page' },
          { url: 'https://github.com/Kaggle/kaggle-cli/blob/main/docs/competitions.md', label: 'Kaggle CLI documentation' }
        ]
      };
    }
  }

  const githubMethod = methods.find(item => item.method === 'github_clone');
  if (githubMethod) {
    const structuredRepoId = String(githubMethod.repo_id || '').trim();
    const repoId = repoPattern.test(structuredRepoId) ? structuredRepoId : githubDatasetRepo(ds);
    if (repoId) {
      const localDir = repoId.split('/').pop() || 'dataset';
      const repoUrl = `https://github.com/${repoId}`;
      return {
        provider: 'GitHub',
        repoId,
        repoUrl,
        description: 'Clone the complete public repository, including its original folder structure and download instructions.',
        tabs: [{
          id: 'cli',
          label: 'CLI',
          help: 'Run this command in the directory where you want to save the repository.',
          command: `git clone ${repoUrl}.git ./${localDir}`,
          copyLabel: 'Copy command'
        }],
        notice: 'Some repositories store large dataset files with Git LFS or link to files hosted elsewhere. Follow the repository README if the clone contains pointers or download scripts.',
        links: [
          { url: repoUrl, label: 'Open repository' },
          { url: 'https://git-scm.com/docs/git-clone', label: 'git clone documentation' }
        ]
      };
    }
  }

  const method = methods.find(item => item.method === 'huggingface_snapshot');
  const structuredRepoId = String(method?.repo_id || '').trim();
  const repoId = repoPattern.test(structuredRepoId) ? structuredRepoId : huggingFaceDatasetRepo(ds);
  if (!repoId) return null;
  const structuredRevision = String(method?.revision || '').trim();
  const revision = /^[A-Za-z0-9._/-]+$/.test(structuredRevision) ? structuredRevision : 'main';
  const localDir = repoId.split('/').pop() || 'dataset';
  ...
  return {
    ...
    repoId,
    repoUrl,
    description: 'This dataset contains multiple files or shards. Use the official Hugging Face tools below to download the complete repository.',
    tabs: [
      {
        id: 'cli',
        label: 'CLI',
        help: 'Recommended for downloading every repository file while preserving the original folder structure.',
        command: [
          'pip install -U huggingface_hub',
          `hf download ${repoId} --repo-type dataset --revision ${revision} --local-dir ./${localDir}`
        ].join('\n'),
        copyLabel: 'Copy command'
      },
      {
        id: 'python',
        label: 'Python',
        help: 'Use this in a Python workflow when you need the complete repository snapshot.',
        command: [
      ...
      {
        id: 'datasets',
        label: 'Datasets library',
        help: 'Use this to load a compatible dataset into Python. It may not preserve the raw repository layout.',
        command: [
          'from datasets import load_dataset',
          '',
          `ds = load_dataset("${repoId}", revision="${revision}")`
        ].join('\n'),
        copyLabel: 'Copy code'
      }
    ],
    notice: 'Gated datasets require access approval and hf auth login before downloading.',
    links: [
      { url: repoUrl, label: 'Open repository' },
      { url: 'https://huggingface.co/docs/huggingface_hub/main/en/guides/download', label: 'Download documentation' },
      { url: 'https://huggingface.co/docs/datasets/loading', label: 'Datasets documentation' }
    ]
  };
}

function downloadInstructionsModalHtml(instructions){
  if (!instructions) return '';
  const commandPanel = ({ id, command, copyLabel = 'Copy command' }) => `
    <div class="download-command-shell">
      <button type="button" class="btn btn-sm btn-outline-secondary download-command-copy" data-copy-code="${escapeHtml(id)}">${escapeHtml(copyLabel)}</button>
      <pre class="mb-0"><code id="${escapeHtml(id)}">${escapeHtml(command)}</code></pre>
    </div>
  `;
  const tabs = Array.isArray(instructions.tabs) ? instructions.tabs : [];
  const tabNav = tabs.length > 1 ? `
    <ul class="nav nav-tabs" id="datasetDownloadInstructionTabs" role="tablist">
      ${tabs.map((tab, index) => `<li class="nav-item" role="presentation"><button class="nav-link${index === 0 ? ' active' : ''}" id="download-${escapeHtml(tab.id)}-tab" data-bs-toggle="tab" data-bs-target="#download-${escapeHtml(tab.id)}-pane" type="button" role="tab" aria-controls="download-${escapeHtml(tab.id)}-pane" aria-selected="${index === 0 ? 'true' : 'false'}">${escapeHtml(tab.label)}</button></li>`).join('')}
    </ul>` : '';
  const tabContent = tabs.map((tab, index) => `
    <div class="${tabs.length > 1 ? `tab-pane fade${index === 0 ? ' show active' : ''}` : ''}" id="download-${escapeHtml(tab.id)}-pane" role="tabpanel"${tabs.length > 1 ? ` aria-labelledby="download-${escapeHtml(tab.id)}-tab" tabindex="0"` : ''}>
      <p class="small text-muted">${escapeHtml(tab.help)}</p>
      ${commandPanel({ id: `datasetDownload${tab.id}Code`, command: tab.command, copyLabel: tab.copyLabel })}
    </div>`).join('');
  const links = (Array.isArray(instructions.links) ? instructions.links : [])
    .map(link => `<a href="${escapeHtml(link.url)}" target="_blank" rel="noopener">${escapeHtml(link.label)}</a>`)
    .join('');
  return `
    <div class="modal fade" id="datasetDownloadInstructionsModal" tabindex="-1" aria-labelledby="datasetDownloadInstructionsTitle" aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered modal-lg modal-dialog-scrollable">
        <div class="modal-content download-instructions-modal">
          <div class="modal-header">
            <div>
              <div class="detail-kicker mb-1">Full dataset retrieval</div>
              <h2 class="modal-title h5 mb-0" id="datasetDownloadInstructionsTitle">Download with ${escapeHtml(instructions.provider)}</h2>
            </div>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body">
            <p class="text-muted mb-3">${escapeHtml(instructions.description)}</p>
            ${tabNav}
            <div class="${tabs.length > 1 ? 'tab-content pt-3' : ''}" id="datasetDownloadInstructionContent">
              ${tabContent}
            </div>
            <div class="alert alert-light border mt-3 mb-0 small">
              ${escapeHtml(instructions.notice)}
            </div>
          </div>
...
function wireDownloadInstructions(root){
  root.querySelectorAll('[data-copy-code]').forEach(button => {
    button.addEventListener('click', async () => {
      const code = root.querySelector(`#${button.dataset.copyCode}`);
      const original = button.textContent;
      const copied = await copyTextValue(code?.textContent || '');
...
async function downloadPublicFile(url, filename, method = 'navigate'){
  if (method !== 'fetch') {
...
function wireLicenseGate(root){
  ...
  const defaultActionLabel = continueBtn?.textContent || 'Continue';
  let pending = null;
  ...
  root.querySelectorAll('[data-license-gate]').forEach(link => {
    link.addEventListener('click', (event) => {
      ...
      pending = {
        url: href,
        action: link.dataset.licenseAction || 'open',
        downloadMethod: link.dataset.downloadMethod || 'navigate',
        filename: link.dataset.downloadFilename || '',
        instructionsTarget: link.dataset.instructionsTarget || ''
      };
      if (checkbox) checkbox.checked = false;
      if (continueBtn) {
        continueBtn.disabled = true;
        continueBtn.textContent = link.dataset.licenseActionLabel || defaultActionLabel;
      }
  ...
  if (continueBtn) {
    continueBtn.addEventListener('click', async () => {
      if (!pending?.url || continueBtn.disabled) return;
      if (pending.action === 'instructions') {
        const instructionsEl = pending.instructionsTarget
          ? root.querySelector(pending.instructionsTarget)
          : null;
        if (!instructionsEl) {
        ...
        const licenseModal = bootstrap.Modal.getInstance(modalEl);
        const showInstructions = () => new bootstrap.Modal(instructionsEl).show();
        if (licenseModal) {
          modalEl.addEventListener('hidden.bs.modal', showInstructions, { once: true });
          licenseModal.hide();
        } else {
          showInstructions();
        }
      ...
      continueBtn.disabled = true;
      continueBtn.textContent = pending.downloadMethod === 'fetch' ? 'Preparing download…' : 'Starting download…';
      try {
        await downloadPublicFile(pending.url, pending.filename, pending.downloadMethod);
        bootstrap.Modal.getInstance(modalEl)?.hide();
      } catch (error) {
        console.error('Dataset download failed', error);
        continueBtn.textContent = 'Download failed — retry';
...
.oc-abs-toggle:hover{ text-decoration:underline; }
    ...
    const mainHero = `
      <style>
        ...
        .license-footer .form-check{ max-width:520px; }
        .license-footer .form-check-label{ color:#334155; font-size:.92rem; }
        .download-instructions-modal{ border:0; border-radius:14px; box-shadow:0 22px 64px rgba(15,46,75,.2); overflow:hidden; }
        .download-command-shell{ position:relative; overflow:hidden; border:1px solid #d8e4ef; border-radius:10px; background:#0f172a; }
        .download-command-shell pre{ padding:1.1rem 1rem; padding-right:7.5rem; overflow:auto; color:#e2e8f0; font-size:.84rem; line-height:1.55; white-space:pre; }
        .download-command-shell code{ color:inherit; }
        .download-command-copy{ position:absolute; top:.7rem; right:.7rem; z-index:1; background:#fff; }
        @media (max-width: 991.98px){
          .meta-row{ grid-template-columns:1fr; gap:.35rem; }
          .ds-body{ padding:20px 18px; }
          .detail-section{ padding:1.05rem 1rem; }
    ...
    root.innerHTML = `
      ...
      ${downloadInstructionsModalHtml(datasetInstructions)}
    `;
